...
export default function Admin() {
  ...
  const uniqueStudents = new Set(
    data?.tasks.flatMap(task =>
      task.rows.map(row => `${row.class_name}:${row.student_no}`)
    ) || []
  ).size;

  return (
    <div className="admin-page min-h-screen p-4 md:p-7">
      <header className="admin-masthead mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
        <div>
          <p className="comic-kicker">教師專用 · 編輯室</p>
          <h1 className="display-title mt-2 text-3xl md:text-5xl">
            歷史探索數據報
          </h1>
        </div>
        <div className="flex gap-2">
          ...
          <a
            href={`${import.meta.env.BASE_URL}cms/index.html`}
        ...
        <main className="admin-login mx-auto mt-10 max-w-2xl">
          <div className="grid md:grid-cols-[0.7fr_1.3fr]">
            <div className="admin-lock">
              ...
              <p className="pixel-label mt-5">成績資料已上鎖</p>
            </div>
            <form onSubmit={loadDashboard} className="paper-texture p-7 md:p-9">
              <h2 className="display-title text-3xl">教師驗證</h2>
              <p className="mt-2 text-sm leading-6 text-ink/70">
                PIN 不會寫入網站程式；每次讀取都由 GAS 驗證後才回傳全班數據。
              </p>
              <label
              ...
              <div className="relative mt-2">
                <KeyRound className="absolute left-3 top-3.5 h-5 w-5 text-ink/55" />
                <input
                  id="admin-pin"
                  type="password"
                  inputMode="numeric"
                  ...
                  className="comic-input h-12 w-full pl-11"
                  placeholder="••••••••"
                  autoComplete="current-password"
                />
              </div>
              {error && (
                <p className="mt-4 border-2 border-red bg-red/10 p-3 text-sm font-black text-red">
                  {error}
                </p>
              )}
              <button
                disabled={loading}
                className="pixel-button pixel-button-teal mt-6 w-full"
              >
                {loading ? (
                ...
                {loading ? "正在翻查檔案…" : "打開成績檔案"}
              </button>
            </form>
          </div>
        </main>
      ) : (
        <main className="mx-auto mt-7 max-w-7xl">
          ...
          <section className="admin-panel mt-5">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b-3 border-ink p-5">
              <div>
                <p className="pixel-label text-red">總覽</p>
                <h2 className="display-title text-2xl">各任務完成情況</h2>
              </div>
              <button
            ...
            <div className="h-[340px] p-5">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  <CartesianGrid strokeDasharray="4 4" stroke="#172A3A55" />
                  <XAxis
                    dataKey="name"
                    tick={{ fontSize: 11, fill: "#172A3A" }}
                  />
                  <YAxis tick={{ fill: "#172A3A" }} />
                  <Tooltip
                    contentStyle={{
                      background: "#F6E7C1",
                      border: "3px solid #172A3A",
                      boxShadow: "4px 4px 0 #172A3A",
                    }}
                  />
                  <Bar
                    dataKey="完成人次"
