...
export default function Admin() {
  ...
  async function synchronizeCatalogue() {
    ...
    try {
      await syncCatalogue();
      setCatalogueNotice("目前網站的題目、標準答案、配分及版本已同步至 Firestore；既有成績不會因此重新批改。");
    } catch (error) {
      setCatalogueNotice(`題目設定同步失敗，請按「重新整理」重試。${error instanceof Error ? error.message : "請檢查網絡連線。"}`);
      throw error;
    }
  }
  async function reloadWorkspace() {
    // A catalogue failure must not prevent teachers from reading submissions.
    const [loaded] = await Promise.allSettled([refresh(), synchronizeCatalogue()]);
  ...
  return <main className="paper-texture min-h-screen p-5 md:p-10"><div className="mx-auto max-w-7xl">
    <div className="flex flex-wrap gap-3"><Link href="/" className="pixel-button pixel-button-paper">返回探索館</Link><button className="pixel-button pixel-button-paper" onClick={()=>void googleLogout()}>登出</button><button disabled={busy} className="pixel-button pixel-button-paper" onClick={()=>void run(reloadWorkspace)}>重新整理</button><a href={`${import.meta.env.BASE_URL}cms/index.html`} className="pixel-button pixel-button-gold text-center">教材與題目管理（CMS）</a></div>
