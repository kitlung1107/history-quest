...
export function ProfileForm({ onDone }: { onDone?: () => void }) {
  ...
  return <section className="paper-texture mx-auto min-h-screen max-w-2xl p-8"><h1 className="display-title text-3xl">{account.profile ? "我的角色" : "建立你的探險角色"}</h1><p className="my-3">{account.user.email}</p>
    <form className="grid gap-5" onSubmit={async e => { e.preventDefault(); setBusy(true); setError(""); try {
      ...
      <label>班別<select required disabled={!!account.profile} className="comic-input block w-full" value={profile.className} onChange={e=>setProfile({...profile,className:e.target.value})}><option value="">選擇班別</option>{profile.className && !CLASS_OPTIONS.includes(profile.className) && <option value={profile.className}>{displayClass(profile.className)}</option>}{CLASS_OPTIONS.map(c=><option key={c}>{c}</option>)}</select></label>
