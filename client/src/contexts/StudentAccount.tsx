...
export function AccountGate({ children, teacherPage = false }: { children: React.ReactNode; teacherPage?: boolean }) {
  ...
  if (!account) return <main className="login-scene">
    <picture className="login-art" aria-hidden="true">
      <source media="(max-width: 600px) and (orientation: portrait)" srcSet={`${import.meta.env.BASE_URL}images/login-mobile.webp`} />
      <source media="(min-width: 601px) and (max-width: 1400px) and (orientation: portrait)" srcSet={`${import.meta.env.BASE_URL}images/login-tablet.webp`} />
      <img src={`${import.meta.env.BASE_URL}images/login-history.webp`} alt="" fetchPriority="high" />
    </picture>
