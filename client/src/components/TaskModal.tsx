...
import { CheckCircle2, Clock3, ExternalLink, Gamepad2, Sparkles, XCircle } from "lucide-react";
      ...
      <DialogContent className="task-dialog max-h-[92dvh] w-[calc(100%-2rem)] sm:max-w-5xl overflow-y-auto p-0">
        <div className={`task-masthead task-${task.accent}`}>
          ...
          <div className="relative z-10 p-6 md:p-9">
            ...
            <div className="mt-5 flex flex-wrap gap-3 text-sm font-black text-ink"><span className="meta-chip"><Clock3 className="h-4 w-4" />約 {task.duration} 分鐘</span><span className="meta-chip">難度 {"★".repeat(task.difficulty)}{"☆".repeat(5 - task.difficulty)}</span>{completed && <span className="meta-chip bg-teal text-white"><CheckCircle2 className="h-4 w-4" />{gameEmbed ? "網站快問已完成" : "已完成"}</span>}</div>
          </div>
        </div>
        <div className="paper-texture p-6 md:p-9">
          {!started ? (
            <div className="mx-auto max-w-2xl py-8 text-center">
              ...
              <p className="mx-auto mt-3 max-w-xl text-ink/70">{gameEmbed ? "進入互動遊戲探索，再完成網站快問。網站只記錄快問成績及任務進度，遊戲內表現不會傳送至教師後台。" : "閱讀資料後完成一題快問。選擇答案的一刻，系統會自動記錄進度，不需要另外提交。"}</p>
            ...
            <div className={`mx-auto ${gameEmbed ? "max-w-none" : "max-w-3xl"}`}>
              ...
              {gameEmbed && <section className="mt-7" aria-label="互動遊戲">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                  <p className="comic-kicker">HTML5 互動關卡</p>
                  <a className="pixel-button pixel-button-teal text-sm" href={gameEmbed} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-4 w-4" />獨立開啟遊戲（新分頁）</a>
                </div>
                <p className="mb-3 text-sm text-ink/75">手機請轉為橫向；畫面太小或未能載入時，可獨立開啟遊戲。先點一下遊戲畫面，再使用方向鍵或 WASD；亦可使用遊戲內的觸控按鈕。重新載入會開始新一局。</p>
                <div className="embed-frame game-embed-frame"><iframe src={gameEmbed} title={`${task.title} 互動遊戲`} sandbox="allow-scripts allow-same-origin allow-forms allow-pointer-lock allow-popups" allow="autoplay; fullscreen" allowFullScreen /></div>
                <p className="mt-4 text-sm font-bold text-ink/75">遊戲成績與下方網站快問分開；教師後台只接收網站快問成績。</p>
              </section>}
              <section className="quiz-panel mt-9" aria-labelledby="quick-question">
                <p className="comic-kicker inline-flex items-center gap-2"><Sparkles className="h-4 w-4" />{gameEmbed ? "網站快問（非遊戲成績）" : "讀畢快問"}</p>
