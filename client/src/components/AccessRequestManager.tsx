...
export default function AccessRequestManager({
  ...
  const [profiles, setProfiles] = useState<
    { id: string; profile: CloudProfile }[]
  >([]);
  ...
  async function refresh() {
    ...
    setRequests(
      rs.docs
        .map(d => ({ ...(d.data() as AccessRequest), email: d.id }))
        .sort(
          (a, b) =>
  ...
  async function review(approve: boolean) {
    if (previewOnly) throw new Error("示範預覽：不會批准或拒絕真實申請。");
    let profile: CloudProfile | undefined;
    if (approve && mode === "new") {
      const cleanName = name.trim(),
        cleanNo = studentNo.trim().toUpperCase();
      if (
        cleanName.length < 2 ||
        cleanName.length > 50 ||
        !isCurrentClass(className) ||
        !/^[A-Z0-9-]{1,12}$/.test(cleanNo)
      )
        throw new Error("請填齊有效的學生名冊資料。");
      profile = {
        name: cleanName,
        className,
        studentNo: cleanNo,
        nickname: cleanName.slice(0, 20),
        avatar: "explorer",
        configured: false,
      };
    }
  ...
  return (
    <section className={embedded ? "mt-4" : "admin-panel my-8 p-6"}>
      {embedded ? (
        <h4 className="display-title text-xl">准用電郵申請</h4>
      ) : (
        <h2 className="display-title text-2xl">准用電郵申請</h2>
      )}
      <p className="my-3">
        請核對申請人的身分。連結現有學生會沿用其角色及所有成績；建立新帳號會新增學生紀錄。
      </p>
      <button
        disabled={busy}
      ...
      <ul className="my-4 max-h-96 overflow-auto">
        {requests.map(r => (
          <li key={r.email} className="my-3 border-b pb-3">
            <p className="break-all">
              {r.email} · {displayClass(r.className)} · {r.studentNo} · {r.name}
            </p>
            <p>
              {r.status === "pending"
                ? "待審批"
      ...
      {selected && (
        <div className="grid gap-3 border-2 p-4">
          <h3 className="break-all">正在審批：{selected}</h3>
          <label>
            批准方式
            <select
              disabled={busy}
            ...
            >
              <option value="link">連結現有學生帳號</option>
              <option value="new">建立新學生帳號</option>
            </select>
          </label>
          {mode === "link" ? (
            <label>
              ...
              >
                <option value="">請核對並選擇學生</option>
                {profiles.map(p => (
                  <option key={p.id} value={p.id}>
                    {displayClass(p.profile.className)} · {p.profile.studentNo}{" "}
                    · {p.profile.name}
                  </option>
                ))}
              </select>
